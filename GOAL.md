...
- `build.zig` and `mise.toml` - build, test, format, and full check surfaces.
...
- [x] **DoD-1** - `moo serve` can start a localhost HTTP server, report its
  ...
  terminal session. Verify by: `mise run test-integration -- -Dtest-filter
  "http api: serve lifecycle"`.
- [x] **DoD-2** - REST workspace and session management supports create, list,
  ...
  Verify by: `mise run test-integration -- -Dtest-filter
...
- [x] **DoD-3** - REST input, screen snapshot, and wait endpoints can drive a
  ...
  using `attach` or stealing the session. Verify by:
  `mise run test-integration -- -Dtest-filter "http api: drive wait screen"`.
...
- [x] **DoD-5** - REST long-poll event endpoints return monotonic cursors for
  ...
  no false progress. Verify by: `mise run build-debug
...
- [x] **DoD-6** - HTTP exposure is localhost-first and guarded by token auth for
  ...
  before touching session state. Verify by: `mise run build-debug
...
- [x] **DoD-8** - The implementation passes the full existing gate plus the new
  ...
  or agent transcript behavior. Verify by: `mise run check`.

## 4. Exit Conditions

- **`DONE`** - all DoD items and all tasks are complete with valid evidence.
- **`BLOCKED-DEP`** - Zig/libghostty/PTY prerequisites are unavailable after
...
**Evidence**
- 2026-06-24: `mise run test -- -Dtest-filter=parse` exited 0; `mise run check` covered the broader CLI and integration matrix.
...
**Evidence**
- 2026-06-24: `mise run test-integration -- -Dtest-filter='http api: serve lifecycle'` exited 0.
...
**Evidence**
- 2026-06-24: `mise run test-integration -- -Dtest-filter='http api: workspace session management'` exited 0.
...
**Evidence**
- 2026-06-24: `mise run test-integration -- -Dtest-filter='http api:'` exited 0; HTTP group covered drive/wait/screen and resize scenarios.
...
**Evidence**
- 2026-06-24: `mise run test-integration -- -Dtest-filter='http api:'` exited 0; HTTP group covered agent transcript scenarios.
...
**Evidence**
- 2026-06-24: `mise run test-integration -- -Dtest-filter='http api:'` exited 0; HTTP group covered event cursor advance and timeout scenarios.
...
**Evidence**
- 2026-06-24: `mise run test-integration -- -Dtest-filter='http api:'` exited 0; HTTP group covered auth and bind safety.
...
**Evidence**
- 2026-06-24: `mise run fmt-check` exited 0.
- 2026-06-24: `mise run test` exited 0.
- 2026-06-24: `mise run check` exited 0 after format check, ReleaseSafe build, unit
...
- **Commands passed:**
  - `mise run test -- -Dtest-filter=parse`
  - `mise run test-integration -- -Dtest-filter='http api: serve lifecycle'`
  - `mise run test-integration -- -Dtest-filter='http api: workspace session management'`
  - `mise run test-integration -- -Dtest-filter='http api:'`
...
- `status`: complete.
- `confidence`: 95 / 90.
- `latest proof`: rerun targeted HTTP coverage and `mise run check` after any code
