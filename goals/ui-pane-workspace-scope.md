...
- 2026-06-17 — Trigger: requested separate-worktree subagent implementation → Wrong action: attempted `git worktree add -b ...` in a sandbox that could not create branch refs → Revision: use isolated `/private/tmp` source copies when `.git` writes are unavailable, then manually review/apply diffs → Correct action: isolate worker edits without relying on shared `.git`, and record that they are copies rather than real worktrees; impact: 3/5.
